...
describe("nicknames", () => {
  test("a new account has none until it picks one", async (server) => {
    ...
    expect(await server.getAccount()).toEqual({ account: "test-a", nickname: null });
    expect(await server.setNickname("  유적왕 ")).toEqual({ account: "test-a", nickname: "유적왕" });
    expect(await server.getAccount()).toEqual({ account: "test-a", nickname: "유적왕" });
  });
  ...
  test("belongs to one account, ignoring case", async (server) => {
    ...
    expect(await server.getAccount()).toEqual({ account: "test-b", nickname: null });
  });

  test("the owner can change the case of their own name", async (server) => {
    server.connect({ account: "test-a" });
    await server.setNickname("hunter");
    expect(await server.setNickname("HUNTER")).toEqual({ account: "test-a", nickname: "HUNTER" });
  });
  ...
  test("renaming frees the old name for someone else", async (server) => {
    ...
    server.connect({ account: "test-b" });
    expect(await server.setNickname("Hunter")).toEqual({ account: "test-b", nickname: "Hunter" });
