# Traitor Hunt — Plan 3: Multiplayer Client Implementation Plan
...
dev 서버(`preview_start {name: "traitor-hunt-dev"}`)를 띄우고 순서대로 확인한다. 각 단계에서 `read_console_messages {onlyErrors: true}`가 비어 있어야 한다.
...
3. 이 저장소(`traitor-hunt`)의 코드를 그 GitLab에 올린다(사용자가 직접 하겠다고 함). Verse8이 `server/`를 빌드해 배포한다.

- [ ] **Step 2: 프로젝트 ID 넣기**

받은 두 줄을 저장소 루트의 `.agent8.lock`에 그대로 저장한다(파일 머리에 "Auto-generated by Agent8" 주석이 있으면 그대로 둔다). `.env`에도 같은 두 줄을 넣는다(개발 서버가 `VITE_` 값을 읽음). `.env`는 `.gitignore`에 추가하지 않는다 — 공개 정보(프로젝트 ID)뿐이고 Verse8 저장소에도 들어 있다. 비밀 토큰은 절대 넣지 않는다.
