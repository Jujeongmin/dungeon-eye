...
5. **종료** — 지정 탈출구 도달(모험가 승) / 모험가 전멸(배신자 승). 판 시간 제한은 없다(2026-09-17 결정).
...
- 모험가 승리: 배신자를 뺀 살아 있는 모험가 전원이 탈출. 모험가가 모두 죽으면 배신자 승. 판 시간 제한은 없다(2026-09-17, 처음 8분 → 20분 → 없음).
...
판 시간 제한은 없고(HUD에는 지난 시간이 보인다), 아래 단계를 차례로 풀어야 출구가 열린다. 단계와 문은 모두가 보는 방 상태(`match.objectives`)에 있다.
...
**모습 (2026-09-17, 에셋만 사용)**
- 구조물·소품은 코드 도형으로 만들지 않고 던전 키트(Decrepit Dungeon LITE) 모델만 쓴다. 불빛과 이름표 글자만 상태 표시에 쓴다.
- 봉인문: 아치 안의 철창(열리면 위로 올라감). 룬 조각: 작은 탁자 위 열쇠 꾸러미(화면 문구도 "열쇠"). 장치: 긴 탁자 위 촛대(켜지면 불빛, 문구 "의식 촛대").
- 제단: 촛대를 올린 탁자, 지키는 범위(6m)를 둘러싼 촛대 10개. 투표 발판: 위에서 떨어지는 바닥 철창(가장자리에 촛대 3개, 위에서 비추는 조명). 출구: 바닥 해치와 초록 불빛.
- 배신자·묶임 표시: 머리 위 글자("배신자"/"묶임").
- 맵 꾸미기(`dressLevel`): 벽 무늬 5종 섞기, 벽 횃불, 복도 아치, 넓은 방의 매달린 사슬, 벽가 사슬 더미. 칸 위치로 정해져 모두 같은 모습을 본다. 매달린 우리(방보다 김)·바닥 변형 B(매끈해 어색함)·바닥 얼룩 데칼(투명도 깨짐)은 뺐다.
- UI도 필요하면 코드 모양 대신 UI 에셋과 글꼴을 쓴다(후보 조사 중).
...
- 정적 맵과 목표물 소품은 모델 부위마다 `InstancedMesh` 하나로 그린다(`staticBatch.ts`). 움직이는 소품(철창, 열쇠, 투표 철창)도 같은 묶음 안에서 행렬만 바꾼다.
- 고정 횃불 빛은 게임 시작 때 조각마다 계산해 인스턴스 색으로 굽는다(벽이 빛을 가림). 실시간 점광원은 6개 슬롯(`LightPool`)을 카메라에 가까운 불빛(횃불, 촛대, 제단, 열쇠, 투표 철창, 출구)에 나눠 준다.
- 변환 때 단순화: 캐릭터 25%, 사슬 15%, 촛대 35%, 소총 50%. 던전 키트의 정점 색(Unity 블렌드 마스크)은 지운다(웹에서 검게 나옴).
- 측정(1280×720, 개발 PC): 그리기 호출 143~1,133번 → 131~209번, 삼각형 42만~60만 → 23만~26만, 초당 47~61 → 56~61프레임.
