...
- 배신자를 찾아내는 방법은 총이 아니라 "몸으로 투표"다(2026-09-17 결정, 세부 규칙은 Plan 4 설계에서 정한다).
  유적 안에 플레이어마다 이름 룬이 새겨진 발판이 있고, 지목될 사람을 뺀 살아 있는 사람의 과반이 그 사람의 발판에 올라서면 지목된다
  (넷 다 살아 있으면 셋 중 둘). 배신자도 발판에 올라 무고한 사람을 몰아갈 수 있다.
  메뉴 투표가 아니라 몸으로 모여야 하므로, 그동안 몬스터(와 빙의한 배신자)의 공격을 버텨야 한다.
  - 지목된 사람이 배신자면: 빙의 능력이 그 판 끝까지 봉인되고 정체가 모두에게 공개된다. 판은 계속된다.
  - 무고한 사람이면: 그 사람이 20초 동안 묶여 움직이지 못하고, 발판은 한동안 잠긴다. 판은 계속된다.
...
- 맵은 `RUINS`(25×13칸, 한 칸 4m): 시작 구역(발판 홀 포함) → 문 1 → 장치 구역 → 문 2 → 제단 구역 → 문 3 → 보스 방(출구 포함).
- 조작: E 상호작용, Q 빙의(배신자), R 빙의 해제, F 탈출, 클릭 사격.

**몸으로 투표 (룬 발판)**
- 시작 구역의 발판 4개가 참가 순서대로 한 사람씩 가리킨다.
- 지목될 사람을 뺀 살아 있는 사람의 과반이 한 발판 위(1.2m)에 5초 동안 서 있으면 지목이 확정된다. 서버 틱(`$roomTick`)이 판정한다.
- 배신자면 정체가 공개되고(`match.revealed`) 그 판 동안 빙의가 봉인된다. 이후 투표는 없다.
- 무고하면 그 사람이 20초 동안 묶이고(이동·사격·상호작용 불가, `match.bound`), 발판이 90초 동안 잠긴다.
- 묶인 배신자도 빙의는 할 수 있다(몸만 묶임).

**봇 (2026-09-17 결정)**
- 봇은 사람처럼 보이게 다듬는다: 반응 지연(0.3~0.8초), 조준 흔들림(거리 따라 명중률 35~85%), 부드러운 회전, 경로 흔들림, 가끔 멈춰 두리번거림.
- 봇은 목표를 차례로 수행한다. 비명을 들으면 소리 난 곳에 가장 가까이 있던 사람을 의심해 그 사람의 발판에 서고, 다른 사람이 1.5초 넘게 서 있는 발판이 있으면 따라가 선다.
